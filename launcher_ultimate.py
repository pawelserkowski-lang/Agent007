...
try:
    ...
    Config.set('input', 'mouse', 'mouse,multitouch_on_demand') # Wyłącza czerwone kropki (prawy klik)
    ...
    from backend import GeminiBrain  # IMPORT MÓZGU
            ...
            MDScrollView:
                MDBoxLayout:
                    ...
                    MDCard:
                        ...
                        MDTextField:
                            id: sys_instr
                            text: "Jesteś Senior Developerem. Twoim celem jest Code Review."
                            mode: "fill"
...
class DebugDruidApp(MDApp):
    ...
    def build(self):
        ...
        load_dotenv()
        try:
            self.brain = GeminiBrain(api_key=os.getenv("GOOGLE_API_KEY"))
        except ValueError:
            print("BRAK KLUCZA API W PLIKU .ENV!")
            return None # Crash safe
    ...
    def on_start(self):
        ...
        if hasattr(self.root.ids, 'btn_model'):
            ...
            self.root.ids.chat_list.add_widget(
                ChatMessageBubble(text="[color=#00ff00]SYSTEM ONLINE.[/color] Mózg podłączony.\nGotowy do analizy kodu.", is_user=False)
            )
    ...
    def _on_file_drop(self, window, file_path, x, y):
        ...
        if path_str not in self.dropped_files:
            ...
            icon = IconLeftWidget(icon="file-code-outline", theme_text_color="Custom", text_color=(0,1,0,1))
            item.add_widget(icon)
            self.root.ids.file_list.add_widget(item)
            
            # Wysłanie powiadomienia do czatu
            self.root.ids.chat_list.add_widget(
                ChatMessageBubble(text=f"[i]Załadowano plik kontekstowy: {os.path.basename(path_str)}[/i]", is_user=True)
            )
    ...
    def _brain_worker(self, text, settings, files):
        try:
            # Re-init model z nowymi ustawieniami
            ...
            response = f"[color=#ff0000]BŁĄD BACKENDU:[/color] {e}"
