"""
FINALNA STRATEGIA NAPRAWY GUI (NO STRING-INTRAPOLATAION).
Skrypt pisuje plik src/gui.py całkowicie "na płasko",
aby Kivy v2.0 na pewno nie zgłoaszal błedów nawiązujących zama "f('...'".
Bezpieczna wersja dla Python 3.13 / Windows Powershell UTF-Encoding.
"""
from pathlib import Path
import sys

# TREŚC CIAGLA bez łączenia w kodzie Runtime
COMPLETE_GUI_CONTENT = r'''
import os
import threading
...
def write_and_info():
    target = Path("src/gui.py")
    if target.exists(): target.unlink() # Niszcymy staroy nieszcznsciwy plik
    with open(target, 'w', encoding='utf-8') as f:
        # Pisanie Raw bez ryzykaa Python String fomatteow
        f.write("# AUTO GENERATED GUI - FULLY FLAT NO F-STRING RISK\n")
        f.write(COMPLETE_GUI_CONTENT)
    print("------------------------------------------------")
    print(" SUCJA OPNOWA FINALOWA - GUI UZUTYWNWE v3 OK")
    print("------------------------------------------------")
    print(" Odpal glowny launchher: python start_system_v2.py") 
    print(" Powodzeini Arcvihitek. Wyzwanie przyjete")
